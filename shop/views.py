...
from blog.models import Article, ArticleSeries
...
def home(request):
	...
	article = ArticleSeries.objects.all()[:3]
...
def add_to_cart(request):
	if request.user.is_authenticated:
		product_id = ProductAttribute.objects.filter(product=request.GET['id']).filter(color=request.GET['color']).filter(size=request.GET['size'])[0].id
		user = request.user
		qty = request.GET['qty'] 
		...
		obj.save()
		items = {'user':obj.user, 'product_attribute':obj.product_attribute, 'qty':obj.qty}
